...
interface EditTableModalProps {
  ...
  onClose: () => void;
  table: any | null;
  onTableUpdated?: () => void; // optional callback to refresh list
}

export function EditTableModal({ isOpen, onClose, table, onTableUpdated }: EditTableModalProps) {
  ...
  useEffect(() => {
    if (table) {
      ...
      setType(table.type);
      setSpending(table.spending);
      setConfirmed(table.confirmed);
    }
  }, [table]);
      ...
      <div className="relative bg-gray-900 border border-purple-500/30 rounded-lg w-full max-w-md shadow-2xl shadow-purple-500/20 animate-in slide-in-from-bottom duration-300">
        <div className="flex items-center justify-between p-6 border-b border-purple-500/30">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl text-white">Edit Table</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-2">Number of Seats *</label>
            <input
              type="number"
              value={seats}
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-2">Expected Spending (€)</label>
            <input
              type="number"
              value={spending}
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="confirmed_edit" className="text-sm text-gray-400">Mark as confirmed</label>
          </div>

          <div className="flex gap-3 pt-4">
            <button type="button" onClick={onClose} className="flex-1 bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition-colors">Cancel</button>
            <button type="submit" className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white px-4 py-2 rounded-lg transition-all hover:scale-105 shadow-lg shadow-purple-500/30">Update Table</button>
          </div>
        </form>
      </div>
    </div>
